...
`Code.gs` 的 `COL` 已對照 `2026名古屋` 分頁的實際表頭（**第 1 列空白、第 2 列欄位名、第 3 列起是資料**）：

| 欄 | A | B | C | D | E | F | G | H | I | J | K |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 表頭 | 時間 | 項目 | 付款者 | 台幣 | 日幣 | 想想（台） | 想想（日） | Yian（台） | Yian（日） | 備註 | 分帳方式 |

K 欄（分帳方式）原本不存在，第一次從 App 新增或修改紀錄時 script 會自動補上表頭。
不補也能用——App 讀不到 splitType 時會從兩人負擔金額的比例反推（50% → 平均、35% → 65:35），
但手動輸入剛好各半的紀錄會被判成「平均」，所以還是讓它存起來比較準。
