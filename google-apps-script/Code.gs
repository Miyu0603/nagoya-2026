 ...
 *
 * ── 存取密鑰 ──
 * 部署權限是「所有人」，而 /exec 網址一定會出現在公開網站的 JS 裡，
 * 所以只要有人打開網頁原始碼就能讀寫這份試算表。
 * 到「專案設定 → 指令碼屬性」新增 APP_KEY，之後所有請求都必須帶上同一把鑰匙。
 * 沒設 APP_KEY 就維持不檢查——可以先安心部署，設定好屬性才開始生效。
 * 鑰匙不寫在前端程式裡，由使用者在 App 輸入一次、存在自己手機上。
 *
 * ── 前端契約（views/CostView.tsx、App.tsx）──
 * 每個請求都要帶 key（GET 放 query、POST 放 body）；不符則回 { error: 'unauthorized' }。
 * GET  ?sheetName=<分頁名>&key=<密鑰>  → { data: [ {rowIndex, date, item, payer, twd, jpy,
...
/**
 * 比對存取密鑰。沒有設定 APP_KEY 時一律放行，
 * 這樣「先部署新版程式、再設定屬性」的順序不會讓記帳中斷。
 */
function checkAuth_(key) {
...
function doGet(e) {
  try {
    ...
    if (!checkAuth_(params.key)) return unauthorized_();
...
function doPost(e) {
  ...
  try {
    ...
    if (!checkAuth_(body.key)) return unauthorized_();
