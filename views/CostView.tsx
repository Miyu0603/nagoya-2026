...
import { postExpense, setAccessKey, clearAccessKey, UnauthorizedError } from '../lib/expenseApi';
...
export const CostView: React.FC<CostViewProps> = ({ expenses, isLoading, fetchError, needsAccessKey, onUnlocked, onRefresh, onAddSuccess }) => {
  const [keyInput, setKeyInput] = useState('');
  const [writeError, setWriteError] = useState<string | null>(null);

  /**
   * 寫入失敗要講清楚。密鑰過期就把存的清掉，讓畫面退回解鎖狀態，
   * 不要讓使用者對著一個「按了沒反應」的按鈕。
   */
  const reportWriteError = (err: unknown, fallback: string) => {
    if (err instanceof UnauthorizedError) {
      clearAccessKey();
      setWriteError('存取密鑰已失效，請重新解鎖');
      onUnlocked(); // 觸發重讀，伺服器會回 unauthorized，畫面切到解鎖表單
      return;
    }
    console.error(fallback, err);
    setWriteError(`${fallback}，請確認網路後再試一次`);
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await postExpense({ action: 'delete', rowIndex: deleteConfirmId });
      setDeleteConfirmId(null);
      onAddSuccess();
    } catch (err) { reportWriteError(err, '刪除失敗'); } finally { setIsDeleting(false); }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try { await postExpense(payload); setShowModal(false); onAddSuccess(); }
    catch (err) { reportWriteError(err, '儲存失敗'); } finally { setIsSubmitting(false); }
  };
  ...
  const handleBatchDelete = async () => {
    ...
    try {
      ...
      onAddSuccess();
    } catch (err) { reportWriteError(err, '批次刪除失敗'); } finally { setIsBatchProcessing(false); }
  };
  ...
  const handleBatchSplitConfirm = async () => {
    ...
    try {
      ...
      setIsSelectMode(false);
      onAddSuccess();
    } catch (err) { reportWriteError(err, '批次更新失敗'); } finally { setIsBatchProcessing(false); }
  };
  ...
  // 這台裝置還沒解鎖：只給解鎖表單，不顯示任何帳目
  if (isConfigured && needsAccessKey) {
    return (
      <div className="pb-4 pt-4 animate-fade-in-soft">
        <div className="bg-ios-card border border-ios-separator rounded-ios-lg p-5 shadow-ios-card">
          <h2 className="text-[17px] font-semibold text-ios-label tracking-tight mb-1.5">記帳需要解鎖</h2>
          <p className="text-[14px] leading-[1.6] text-ios-label-2 mb-4">
            這個網站是公開的，所以消費紀錄用一組密鑰保護。
            輸入一次就會記在這支手機上，之後不用再輸入。
          </p>
          {writeError && <div className="mb-4"><InfoBanner variant="danger">{writeError}</InfoBanner></div>}
          <form onSubmit={handleUnlock} className="flex gap-2.5">
            <label htmlFor="access-key" className="sr-only">存取密鑰</label>
            <input
              id="access-key"
              type="password"
              value={keyInput}
              ...
              autoComplete="off"
              placeholder="存取密鑰"
              className="flex-1 min-w-0 bg-ios-fill-4 px-3 py-[11px] rounded-ios text-[16px] text-ios-label placeholder:text-ios-label-3 outline-none focus:ring-2 focus:ring-ios-blue"
            />
            <button
              type="submit"
              disabled={!keyInput.trim()}
              className="flex-none px-5 py-[11px] rounded-ios bg-ios-blue text-white text-[16px] font-semibold disabled:opacity-40"
            >
              解鎖
            </button>
          </form>
  ...
  return (
    <div className="pb-4 pt-4 animate-fade-in-soft">
      ...
      {isConfigured && writeError && (
        <div className="mb-5">
          <InfoBanner variant="danger">{writeError}</InfoBanner>
        </div>
      )}
        ...
        <form onKeyDown={(e) => { if (e.key === 'Enter') e.preventDefault(); }} onSubmit={handleSubmit} className="px-5 pb-5 space-y-4">
          ...
          <div className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-3 items-end">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <div className="flex h-[46px] bg-ios-fill-3 p-[3px] rounded-ios">
                <button type="button" onClick={() => setPayer('想想')}
                  className="flex-1 text-[15px] font-semibold rounded-tk-sm transition-all"
          ...
          <div>
            <label className="text-[13px] font-semibold text-ios-label-2 mb-1.5 block">內容</label>
            <input type="text" placeholder="輸入消費內容" value={item} onChange={e => setItem(e.target.value)}
              className="w-full bg-ios-fill-4 px-3 py-2.5 rounded-ios text-[16px] font-medium text-ios-label outline-none placeholder:text-ios-label-3" />
          </div>

          <div>
            <label className="text-[13px] font-semibold text-ios-label-2 mb-1.5 block">金額</label>
              ...
              {SPLIT_OPTIONS.map((opt, i) => (
                ...
                >
                  <input
                    ...
                    className="sr-only"
                  />
                  <span className={`block py-2.5 text-[14px] font-semibold select-none ${splitType === opt.value ? 'bg-mag-gold text-white' : 'bg-white text-ios-label-2'}`}>
                    {opt.label}
                  </span>
                </label>
              ))}
          ...
          <div>
            <label className="text-[13px] font-semibold text-ios-label-2 mb-1.5 block">備註</label>
